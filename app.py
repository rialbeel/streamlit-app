...
if uploaded_file is not None:
    ...
    if np.issubdtype(df[choice].dtype, np.number):
        ...
        fig = sns.displot(df, x = choice, kde = True, color = "darkblue")
        plt.xlabel(choice)
    ...
    if np.issubdtype(df[choice].dtype, np.object_):
        ...
        st.pyplot(fig)
    if np.issubdtype(df[choice].dtype, np.bool_):
        counts = df[choice].value_counts(normalize = True)
        prop = pd.DataFrame({"Value": counts.index, "Proportion": counts.values})
        st.write(prop)
        fig = plt.figure()
        sns.histplot(x = df[choice].astype("int64"), stat = "proportion", bins = 2, hue = df[choice], palette = ("red", "green"))
        plt.xlabel(choice)
        plt.ylabel("Proportion")
        plt.title(f"Barplot of {choice}")
        st.pyplot(fig)